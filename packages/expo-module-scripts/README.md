...
- [`with-node.sh`](./templates/scripts/with-node.sh): Another Xcode build phase script helpers for Node.js binary resolution. To load Node.js binary from react-native project's `.xcode.env` or `.xcode.env.local` files.
